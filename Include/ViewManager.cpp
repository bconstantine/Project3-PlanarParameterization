...
void ViewManager::mouseReleaseEvent(int button, int x, int y)
{
	...
	else if(button == 92 || button == 3 || button == 4) {
        midDown = false;
    }
}
...
* @param y 滑鼠在畫面的y座標值。
*/
void ViewManager::mouseMoveEvent(int button, int x,int y)
{
    if(button == 90)
	{
		/*
	...
	else if(button == 92)
	{
        ...
        translationMatrix = translate(translationMatrix, (-diffUp + diffRight) * zoom * 0.5f);
        midDownCoord = coord;
    }
}
...
void ViewManager::Zoom(float distance)
{
    ...
    zoom = clamp(0.1f, zoom, 10.0f); //before is 3
}
